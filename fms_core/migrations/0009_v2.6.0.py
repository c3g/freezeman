# Generated by Django 3.1 on 2020-11-18 14:57
...
def populate_foreign_keys(apps, schema_editor):
    ...
    all_container_uuids = set(container_model.objects.all().values_list("old_id", flat=True))
    old_to_new_id_map = dict(container_model.objects.all().values_list("old_id", "id"))
    ...
    for version in version_model.objects.filter(content_type__model="container", object_id__in=all_container_uuids):
        ...
        version.object_id = old_to_new_id_map.get(old_id)
        # Re-serialize data to fit new model
        data = json.loads(version.serialized_data)
        data[0]["pk"] = version.object_id
        data[0]["fields"]["location"] = old_to_new_id_map.get(data[0]["fields"]["location"])
    ...
    for version in version_model.objects.filter(content_type__model="sample"):
        ...
        data = json.loads(version.serialized_data)
        data[0]["fields"]["container"] = old_to_new_id_map.get(data[0]["fields"]["container"])
        version.serialized_data = json.dumps(data)
        # Save to database
        version.save()
    ...
    for log in LogEntry.objects.filter(content_type__model="container", object_id__in=all_container_uuids):
        log.object_id = old_to_new_id_map.get(log.object_id)
        # Save to database
        log.save()
