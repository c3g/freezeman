...
def fetch_sample_data(ids: List[int] =[], queryset=None, query_params=None) -> List:
    ...
    try:
        sample_values_queryset = (
            ...
            #.annotate(process_measurements=Subquery(ProcessMeasurement.objects.filter(source_sample=OuterRef("pk")).values_list("process_measurement", flat=True)))
    ...
    if not samples_queryset:
        ...
        for sample in samples.values():
            ...
            data = {
                ...
                'is_pool': is_pool,
                'project': derived_sample["project"] if not is_pool else None,
                'process_measurements': [sample["process_measurement"]] if sample["process_measurement"] is not None else [],
    ...
    if not libraries_queryset:
        ...
        for sample in samples.values():
            ...
            data = {
                ...
                'concentration_nm': None, #sample["concentration_as_nm"],
