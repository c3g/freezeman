...
from fms_core.coordinates import convert_alpha_digit_coord_to_ordinal, ROW_ORDINAL_COORDINATE_ALLOCATION, COLUMN_ORDINAL_COORDINATE_ALLOCATION
...
class RunInfoSample:
    ...
    external_index_name: Optional[str] = None
...
class RunInfo:
    ...
    run_name: str
    external_run_name: str          # Run ID given by the instrument
    run_obj_id: int
...
def generate_run_info(experiment_run: ExperimentRun) -> Dict[str, Any]:
    ...
    run_info : RunInfo = RunInfo(
        ...
        external_run_name=experiment_run.external_name or _get_external_run_name(experiment_run),
...
def _generate_pooled_samples(experiment_run: ExperimentRun, pool: Sample) -> List[RunInfoSample]:
    ...
    for derived_sample in derived_samples:
        ...
        if project is None:
            ...
            run_info_sample.external_project_id = project.parent_project.external_id
            run_info_sample.external_project_name = project.parent_project.name
...
def _generate_sample(experiment_run: ExperimentRun, sample: Sample, derived_sample: DerivedSample) -> RunInfoSample:
    ...
    row.lane = convert_alpha_digit_coord_to_ordinal(sample.coordinates, container_spec.coordinate_spec, axis=container_spec.ordinal_coordinates_allocation_axis)
    ...
    if derived_sample.library is not None:
        ...
        row.external_index_name = index.external_name
...
def _get_external_run_name(experiment_run : ExperimentRun) -> str:
    """
    Experiment run identifier generated by the instrument. This information may be stored in as a property on process linked to the Experiment Run model.
    The property name may differ from one platform to the other. The function returns the External Run Name linked to the given Experiment Run if one exists.
    Args:
        `experiment_run`: An experiment run object, which may or may not have an external run name.
    Returns:
        An External Run Name string, or None.
    """
    # DNBSEQ case
