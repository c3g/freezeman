...
def get_step_from_template(protocol, template_sheets, template_sheet_definition, batch_centric=False):
    """
    ...
    Args:
        ...
        `batch_centric`: Template is batch centric. The match to step must be done using the batch sheet instead of the sample sheet.
                         Defaults to False, for sample centric.
    ...
    if not batch_centric: # Sample centric
        ...
        # get batch sheet : the first one
        batch_sheet_list = [sheet["name"] for sheet in template_sheet_definition if sheet.get("batch", False)]
        batch_sheet_name = batch_sheet_list.pop(0) # get only a single name, the first listed in the sheet definition

        if len(candidate_steps) == 1:
            matching_step = candidate_steps.first()
            matching_dict = {row_id: matching_step for row_id, _ in enumerate(template_sheets[batch_sheet_name].rows)}
            return matching_dict, errors, warnings
        elif not candidate_steps:
            errors.append(f"No step matches the given protocol.")
            return matching_dict, errors, warnings
            ...
            first_pass = True # To prevent duplicated error reporting.
            for candidate_step in candidate_steps:
                batch_sheet_matches = defaultdict(list)
                for step_specification in candidate_step.step_specifications.all():
                    sheet = template_sheets[step_specification.sheet_name]
                    for row_id, row_data in enumerate(sheet.rows):
                        ...
                        if step_specification.sheet_name == batch_sheet_name:
                            ...
                            errors.append(f"Template data does not match step specifications.")
                work_dict[candidate_step] = {row_id: batch_sheet_matches[row_id] for row_id, _ in enumerate(template_sheets[batch_sheet_name].rows)}
                first_pass = False
            # build the return dict from work dict
            for candidate_step, rows in work_dict.items():
                for row_id, matches in rows.items():
