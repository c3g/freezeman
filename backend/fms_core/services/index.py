...
def update_library_index(derived_sample_obj: DerivedSample, new_index_name: str, old_index_name: str = None):
    """
    Changes the current index for all library instances in the lineage of the provided derived_sample.
    The new_index_name is required and need to match an existing index, but the old_index_name is optional and provide a way to validate the request.

    Args:
        `derived_sample_obj`: derived sample object for the library to correct. Will apply the change to all derived samples in its lineage that are libraries.
        `new_index_name`: string with the name of the index that will replace the current index.
        `old_index_name`: string with the name of the current index for validation purpose. Defaults to None when validation is not required.

    Returns:
        Tuple including a list of sample instance affected by the update as well as a list of errors and warnings.
    """
    errors = []
    warnings = []
    ...
    try:
        # get the new index
        index_obj = Index.objects.get(name=new_index_name)
        if old_index_name is not None:
            try:
                old_index_obj = Index.objects.get(name=old_index_name)
            except Index.DoesNotExist:
                errors.append(f"Index name provided for validation does not exist.")
            if derived_sample_obj.library.index_id != old_index_obj.id:
                errors.append(f"Library index does not match the index provided for validation.")
        if not errors: 
            ...
            current_derived_sample = derived_sample_obj
            ...
            for derived_sample_to_update in derived_sample_lineage:
                _, index_update_errors, index_update_warnings = update_library(derived_sample=derived_sample_to_update, index=index_obj)
                errors.extend(index_update_errors)
                warnings.extend(index_update_warnings)

            if len(derived_sample_lineage) > 1:
                warnings.append(f"A total of {len(derived_sample_lineage)} derived libraries that share lineage with the updated library were changed.")
                
            samples_impacted = [sample for sample in Sample.objects.filter(derived_samples__in=derived_sample_lineage).distinct()]
    except Index.DoesNotExist:
        errors.append(f"No index found for name [{new_index_name}].")
    except Index.MultipleObjectsReturned:
        errors.append(f"Multiple indices found for name [{new_index_name}]. Should be unique.")
    
    return samples_impacted, errors, warnings