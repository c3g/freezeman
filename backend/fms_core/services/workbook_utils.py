...
def insert_cells(worksheet: Worksheet, first_cell_location: tuple[int, int], descriptors: Iterable[Iterable[CellDescription | Any]], order: Literal["row"] | Literal["col"]):
    validations = set()
    ...
    for outer_offset, descriptor_row in enumerate(descriptors):
        for inner_offset, cell_description in enumerate(descriptor_row):
            ...
            if not isinstance(cell_description, CellDescription):
                column_letter = get_column_letter(first_cell_location[1] + column_offset)
                worksheet[f"{column_letter}{first_cell_location[0] + row_offset}"] = cell_description
            else:
