...
def update_library_index(derived_sample_obj: DerivedSample, new_index_obj: Index, old_index_obj: Index = None):
    """
    ...
    Args:
        ...
        `new_index_obj`: index object that will replace the current index.
        `old_index_obj`: current index object for validation purpose. Defaults to None when validation is not required.
    ...
    if not errors: 
        ...
        for derived_sample_to_update in derived_sample_lineage:
            _, index_update_errors, index_update_warnings = update_library(derived_sample=derived_sample_to_update, index=new_index_obj)
            errors.extend(index_update_errors)
            warnings.extend(index_update_warnings)

        if len(derived_sample_lineage) > 1:
            warnings.append(f"A total of {len(derived_sample_lineage)} libraries that share lineage with the updated library were changed.")
