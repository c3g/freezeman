...
def create_sample_lineage_graph(sampleId: int) -> Tuple[List[Dict], List[Dict], List[str]]:
    ...
    Args:
        `sampleId` (`int`): ID of an existing sample

    Returns:
        `Tuple[List[Dict], List[Dict], List]` consisting of
        nodes, edges and errors.

        A node consists of sample `"name"`, `"quality_flag"` and `"quantity_flag"`.

        An edge consists of process `"id"`, `"source_sample"`, `"child_sample"`, `"protocol_name"`
