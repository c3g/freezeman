...
from fms_core.template_importer.row_handlers.library_preparation_with_selection import LibraryRowHandler, LibraryBatchRowHandler
from fms_core.templates import LIBRARY_PREPARATION_WITH_SELECTION_TEMPLATE
...
class LibraryPreparationWithSelectionImporter(GenericImporter):
    ...
    def initialize_data_for_template(self):
        ...
        protocol = Protocol.objects.get(name='Library Preparation with Selection')
    ...
    def import_template_inner(self):
        ...
        for row_id, row in enumerate(library_batch_sheet.rows):
            ...
            (result, batch_objects) = self.handle_row(
                ...
                library_selection=str_cast_and_normalize(library_batch_dict['Library Selection']),
                library_selection_target=str_cast_and_normalize(library_batch_dict['Library Selection Target']),
            ...
            if any(library_batch_sheet.rows[row_id]): # Add the data only if the row is not empty
                library_batch_info = dict(
                    ...
                    # Library Type, Library Selection, and Platform
                    **batch_objects,
