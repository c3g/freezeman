...
from fms_core.models import InstrumentType, Process, Step, SampleNextStep
...
class QCIntegrationSparkRowHandler(GenericRowHandler):
    ...
    def process_row_inner(self,
        ...
        if sample_obj:
            ...
            queued_to_workflow_step = SampleNextStep.objects.filter(sample_id=sample_obj.id, step_id=workflow["step"].id).exists()

            if not queued_to_workflow_step:
                self.errors['source_sample'].append(f"Sample is not queued to the workflow step {workflow['step'].name}.")
            else:
