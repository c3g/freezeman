...
class SampleRowHandler(GenericRowHandler):
    ...
    def process_row_inner(self, template_data, additional_data):
        # template data = sample_type, sample, library, container, project, parent_container, individual
        # additional_data = sample_kind_objects_by_name, defined_pools
        comment = template_data["sample"]['comment'] if template_data["sample"]['comment'] else f"Automatically generated via Sample submission Template on {datetime.now(timezone.utc).isoformat()}Z"

        # Individual related section
        taxon_obj = None
        if template_data["individual"]['taxon']:
            taxon_obj, self.errors['taxon'], self.warnings['taxon'] = get_taxon(name=template_data["individual"]['taxon'])

        reference_genome_obj = None
        if template_data["individual"]['reference_genome']:
            reference_genome_obj, self.errors['reference_genome'], self.warnings['reference_genome'] = get_reference_genome(assembly_name=template_data["individual"]['reference_genome'])

        individual_obj = None
        need_individual = any([template_data["individual"]["taxon"],
                               template_data["individual"]["sex"],
                               template_data["individual"]["cohort"],
                               template_data["individual"]["alias"],
                               template_data["individual"]["reference_genome"]])
        ...
        if not template_data["individual"]["name"] and need_individual and not can_use_generic_individual:
            ...
            if template_data["individual"]["sex"] is None:
                template_data["individual"]["sex"] = Individual.SEX_UNKNOWN
            if not template_data["individual"]["name"]:
                template_data["individual"]["name"] = Individual.GENERIC_INDIVIDUAL_PREFIX + str(get_unique_id())
                is_generic = True
            else:
            ...
            individual_obj, created, self.errors['individual'], self.warnings['individual'] = \
                get_or_create_individual(name=template_data["individual"]['name'],
                                         alias=template_data["individual"]['alias'],
                                         taxon=taxon_obj,
                                         sex=template_data["individual"]['sex'],
                                         cohort=template_data["individual"]['cohort'],
        ...
        try:
            sample_kind_obj = additional_data["sample_kind_objects_by_name"][template_data["sample"]['sample_kind']]
        except KeyError as e:
            self.errors['sample_kind'] = [f"Sample Kind {template_data['sample']['sample_kind']} not found."]
        #TODO: sample kind str normalization
        tissue_source_obj = None
        if template_data["sample"]['tissue_source']:
            try:
                tissue_source_obj = additional_data["sample_kind_objects_by_name"][template_data["sample"]['tissue_source']]
            except KeyError as e:
                self.errors['tissue_source'] = [f"Tissue source {template_data['sample']['tissue_source']} not found."]

        # Library are submitted
        library_obj = None
        is_library = any([template_data["library"]['library_type'], template_data["library"]['index'], template_data["library"]['platform'], template_data["library"]['strandedness']])
        if is_library:
            # Create library objects
            library_type_obj, self.errors['library_type'], self.warnings['library_type'] = get_library_type(template_data["library"]['library_type'])
            index_obj, self.errors['index'], self.warnings['index'] = get_index(template_data["library"]['index'])
            platform_obj, self.errors['platform'], self.warnings['platform'] = get_platform(template_data["library"]['platform'])
            library_selection_obj = None
            if template_data["library"]['selection_name'] and template_data["library"]['selection_target']:
                library_selection_obj, self.errors['library_selection'], self.warnings['library_selection'] = \
                    get_library_selection(name=template_data["library"]['selection_name'], target=template_data["library"]['selection_target'])
            elif bool(template_data["library"]['selection_name']) != bool(template_data["library"]['selection_target']):
            ...
            library_obj, self.errors['library'], self.warnings['library'] = create_library(library_type=library_type_obj,
                                                                                           ...
                                                                                           strandedness=template_data["library"]['strandedness'],
                                                                                           library_selection=library_selection_obj)

        # Project related section
        project_obj = None
        studies_obj = []
        if template_data["project"]['name']:
            project_obj, self.errors['project'], self.warnings['project'] = get_project(template_data["project"]['name'])

            if project_obj and template_data["project"]['study_letter']:
                  study_letters = [s.strip() for s in template_data["project"]['study_letter'].split("-") if s != ""]
        ...
        if template_data["library"]['pool_name'] is None:
            # Output warning if the there is already a sample with the same name
            if Sample.objects.filter(name__exact=template_data["sample"]['name']).exists():
                self.warnings['name'] = ('Sample with the same name [{0}] already exists. A new sample with the same name will be created.', [template_data["sample"]["name"]])

            # Container related section
            parent_container_obj = None
            if template_data["parent_container"]['barcode']:
                parent_container_obj, _, self.errors['parent_container'], self.warnings['parent_container'] = \
                    get_or_create_container(barcode=template_data["parent_container"]['barcode'],
                                            kind=template_data["parent_container"]['kind'],
                                            name=template_data["parent_container"]['name'],
                                            creation_comment=comment)

            container_obj, _, self.errors['container'], self.warnings['container'] = \
                get_or_create_container(barcode=template_data["container"]['barcode'],
                                        kind=template_data["container"]['kind'],
                                        name=template_data["container"]['name'],
                                        coordinates=template_data["container"]['coordinates'],
            ...
            if library_obj is not None or not is_library:
                sample_obj, self.errors['sample'], self.warnings['sample'] = \
                    create_full_sample(name=template_data["sample"]['name'], volume=template_data["sample"]['volume'], collection_site=template_data["sample"]['collection_site'],
                                       creation_date=template_data["sample"]['creation_date'], coordinates=template_data["sample"]['coordinates'], alias=template_data["sample"]['alias'],
                                       concentration=template_data["sample"]['concentration'], tissue_source=tissue_source_obj,
                                       experimental_group=template_data["sample"]['experimental_group'], container=container_obj, individual=individual_obj,
        ...
        elif template_data["library"]['pool_name'] and not library_obj:
            self.errors['pooling'] = [f"A valid library is necessary to pool this sample."]
        # for pools
        else:
            template_data["sample"]['alias'] = template_data["sample"]['alias'] or template_data["sample"]['name']
            if not template_data["sample"]['alias']:
                ...
                try:
                    name_validator_without_dot(template_data["sample"]['alias'])
                except ValidationError as e:
                    self.errors['alias'] = [f"Sample Name or Alias {template_data['sample']['alias']} is not valid."]
            if additional_data["defined_pools"].get(template_data["library"]['pool_name'], None) is None:
                self.errors['pooling'] = [f"Pool {template_data['library']['pool_name']} for the library in pool {template_data['sample']['name']} is not defined on the PoolSubmission sheet."]

        # For pooling purposes
        self.row_object = {
            "sample_type": template_data["sample"]["sample_type"],
            # Biosample info
            "alias": template_data["sample"]['alias'],
            "individual": individual_obj,
            "collection_site": template_data["sample"]['collection_site'],
            # Derived sample info
            "sample_kind": sample_kind_obj,
            "tissue_source": tissue_source_obj,
            "experimental_group": template_data["sample"]['experimental_group'],
            "library": library_obj,
            "project": project_obj,
            "studies": studies_obj,
            # Pool relation info
            "volume_ratio": template_data["sample"]['volume_ratio'],
        }