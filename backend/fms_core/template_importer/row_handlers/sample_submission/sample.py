...
class SampleRowHandler(GenericRowHandler):
    ...
    def process_row_inner(self, sample, library, container, project, parent_container, individual, individual_mother, individual_father, sample_kind_objects_by_name):
        ...
        if individual['taxon']:
            ...
            self.warnings['taxon'] = [(w,) for w in self.warnings['taxon']]
        ...
        if individual['reference_genome']:
            ...
            self.warnings['reference_genome'] = [(w,) for w in self.warnings['reference_genome']]
        ...
        if individual_mother['name']:
            ...
            self.warnings['individual_mother'] = [(w,) for w in self.warnings['individual_mother']]
        ...
        if individual_father['name']:
            father_obj, _, self.errors['individual_father'], self.warnings['individual_father'] = \
                get_or_create_individual(name=individual_father['name'],
                                         ...
                                         pedigree=individual["pedigree"],
                                         reference_genome=reference_genome_obj)
            self.warnings['individual_father'] = [(w,) for w in self.warnings['individual_father']]
        ...
        if not individual["name"] and need_individual:
            ...
            individual_obj, created, self.errors['individual'], self.warnings['individual'] = \
                get_or_create_individual(name=individual['name'],
                                         ...
                                         mother=mother_obj,
                                         father=father_obj)
            self.warnings['individual'] = [(w,) for w in self.warnings['individual']]
            if not created:
        ...
        try:
            ...
            self.errors['sample_kind'] = [("Sample Kind {0} not found.", sample['sample_kind'])]
        ...
        if sample['tissue_source']:
            try:
                ...
                self.errors['tissue_source'] = [("Tissue source {0} not found.", sample['tissue_source'])]
        ...
        if is_library:
            ...
            self.warnings['library_type'] = [(w,) for w in self.warnings['library_type']]
            index_obj, self.errors['index'], self.warnings['index'] = get_index(library['index'])
            self.warnings['index'] = [(w,) for w in self.warnings['index']]
            platform_obj, self.errors['platform'], self.warnings['platform'] = get_platform(library['platform'])
            self.warnings['platform'] = [(w,) for w in self.warnings['platform']]
            ...
            if library['selection_name'] and library['selection_target']:
                ...
                self.warnings['library_selection'] = [(w,) for w in self.warnings['library_selection']]
            ...
            self.warnings['library'] = [(w,) for w in self.warnings['library']]
        ...
        if project['name']:
            ...
            self.warnings['project'] = [(w,) for w in self.warnings['project']]
            ...
            if project_obj and project['study_letter']:
                  ...
                  for study_letter in study_letters:
                      ...
                      self.warnings['study'].extend([(w,) for w in study_warnings])
        ...
        if library['pool_name'] is None:
            ...
            if parent_container['barcode']:
                ...
                self.warnings['parent_container'] = [(w,) for w in self.warnings['parent_container']]
            ...
            self.warnings['container'] = [(w,) for w in self.warnings['container']]
            ...
            if library_obj is not None or not is_library:
                ...
                self.warnings['sample'] = [(w,) for w in self.warnings['sample']]
            ...
            if project_obj and sample_obj:
                ...
                self.warnings['project_link'] = [(w,) for w in self.warnings['project_link']]
                for study_obj in studies_obj:
                    ...
                    self.warnings['queue_to_study'].extend([(w,) for w in queue_warnings])
