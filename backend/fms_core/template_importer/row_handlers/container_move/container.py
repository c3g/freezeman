...
class ContainerRowHandler(GenericRowHandler):
    ...
    def process_row_inner(self, container, destination_container):
        ...
        comment = f"Automatically generated via Container move Template on {datetime.utcnow().isoformat()}Z"
        ...
        if not all([destination_container['destination_barcode'], destination_container['destination_coordinates']]):
            self.errors['container_move'] = f'Destination location barcode and destination location coordinates are required.'
            return
