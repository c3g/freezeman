# Generated by Django 3.1 on 2021-10-26 14:26
...
class Migration(migrations.Migration):
    ...
    operations = [
        ...
        migrations.RunSQL(
            """
                -- create derived_sample and derived_by_sample
                -- WIP: To fix!
                WITH derived_sample_ids AS (
                    INSERT INTO fms_core_derivedsample (sample_kind_id, experimental_group, tissue_source, biosample_id, created_at, created_by_id, updated_at, updated_by_id, deleted)
                    SELECT sample.sample_kind_id, sample.experimental_group, sample.tissue_source, biosample.id, sample.created_at, sample.created_by_id, current_timestamp, 1, FALSE
                    FROM fms_core_sample sample
                    LEFT JOIN fms_core_biosample biosample
                        ON biosample.name = sample.name
                        AND biosample.individual_id = sample.individual_id
                        AND biosample.collection_site = sample.collection_site
                    RETURNING id AS derived_sample_id, sample.id AS sample_id
                )
                ...
                SELECT derived_sample_id, sample_id, 1, current_timestamp, 1, current_timestamp, 1, FALSE  FROM derived_sample_ids;             
            """,
            migrations.RunSQL.noop
        ),
