...
export function coordinatesToOffsets(spec: CoordinateSpec, coordinates: string) {
  ...
  for (const axis of spec) {
    if (coordinates.length === 0) {
      throw new Error(
        `Cannot convert coordinates ${originalCoordinates} with spec ${JSON.stringify(spec)} to offsets at axis ${axis}`,
      )
    }
...
export function offsetsToCoordinates(offsets: readonly number[], spec: CoordinateSpec) {
  if (spec.length !== offsets.length) {
    throw new Error(
      `Cannot convert offsets ${JSON.stringify(offsets)} to coordinates with spec ${JSON.stringify(spec)}`,
    )
  }
  ...
  for (let i = 0; i < spec.length; i++) {
    ...
    if (offsets[i] >= spec[i].length) {
      throw new Error(
        `Cannot convert offset ${JSON.stringify(offsets)} to coordinates with spec ${JSON.stringify(spec)} at axis ${i}`,
      )
    }
...
export function comparePlacementSamples(
  a: PlacementSample,
  b: PlacementSample,
  spec?: ContainerKind["coordinate_spec"],
) {
  const MAX = 1
...
export function smartQuerySetLookup(
  field: string,
