import { clearFilters, removeFilter, setFilterFixed, setFilterOptions, setFilterValue } from "./filter_set_reducers"
...
export function reduceSetFixedFilter<P extends PagedItems>(pagedItems: P, filter: FilterSetting): P {
    if (filter.description) {
        return {
            ...pagedItems,
            fixedFilters: setFilterFixed(pagedItems.filters, filter.description, true),
        }
    } else {
        return pagedItems
    }
...
export function reduceClearFixedFilters<P extends PagedItems>(pagedItems: P): P {
    return {
        ...pagedItems,
        filters: Object.values(pagedItems.filters).reduce((filterset, filter) => {
            if (filter && filter.description) {
                return setFilterFixed(filterset, filter.description, false)
            } else {
                return filterset
            }
        }, pagedItems.filters)
