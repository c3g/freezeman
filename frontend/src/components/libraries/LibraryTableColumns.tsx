...
} from "../shared/WithItemRenderComponent"
...
export const LIBRARY_COLUMN_DEFINITIONS: { [key in LibraryColumnID]: LibraryColumn } = {
  [LibraryColumnID.CONCENTRATION_NM]: {
    ...
    render: (conc) =>
      isNullish(conc) ? (
        ...
        <Typography className="table-column-numbers" style={{ float: "right" }}>
          {parseFloat(conc).toFixed(3)}
        </Typography>
      ),
    sorter: { multiple: 1 },
  },
  ...
  [LibraryColumnID.LIBRARY_TYPE]: {
    ...
    render: (_, { library }) =>
      library && library.library_type && <span>{library.library_type}</span>,
    width: 150,
    sorter: { multiple: 1 },
  },
  ...
  [LibraryColumnID.SELECTION_TARGET]: {
    ...
    render: (_, { library }) =>
      library &&
      library.library_selection_target && <span>{library.library_selection_target}</span>,
  },
  ...
  [LibraryColumnID.NAME]: {
    ...
    render: (name, { library }) =>
      library && <Link to={`/samples/${library.id}`}>{library.name}</Link>,
    sorter: { multiple: 1 },
  },
      ...
      return (
        ...
        library.container && (
          <WithContainerRenderComponent
            ...
            placeholder={<span>loading...</span>}
            render={(container) => (
              <Link to={`/containers/${library.container}`}>{container.barcode}</Link>
            )}
          />
        )
      )
  ...
  [LibraryColumnID.QC_FLAG]: {
    ...
    render: (_, { library }) => {
      if (library) {
        const flags = {
          quantity: library.quantity_flag,
          quality: library.quality_flag,
          identity: library.identity_flag,
        }
...
export const SAMPLE_NEXT_STEP_BY_STUDY_LIBRARY_FILTER_KEYS = {
  [LibraryColumnID.LIBRARY_TYPE]:
    "sample_next_step__sample__derived_samples__library__library_type__name",
  [LibraryColumnID.SELECTION_TARGET]:
    "sample_next_step__sample__derived_samples__library__library_selection__target",
  [LibraryColumnID.INDEX_NAME]: "sample_next_step__sample__derived_samples__library__index__name",
  [LibraryColumnID.PLATFORM_NAME]:
    "sample_next_step__sample__derived_samples__library__platform__name",
