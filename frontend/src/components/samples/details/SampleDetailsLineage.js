...
const SampleDetailsLineage = ({
      ...
      Array.from(graphs.values()).reduce(({ nodes, links }, g) => {
        ...
        return {
          ...
          links: [
            ...links,
            ...children
              .filter(([_, child_graph]) => graphs.has(child_graph.data.id))
              .map(([process, child_graph]) => {
                ...
                return {
                  ...
                  source: curr_sample.id,
                  target: child_sample.id,
                }
              }),
          ]
        }
  ...
  }, [samplesByID, processMeasurementsByID, protocolsByID, sample])
