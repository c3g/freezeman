...
import { Row, Col, Timeline, Empty, TimelineItemProps } from "antd"
...
export default function ArchivedCommentsTimeline({ comments } : commentsTimelineProps) {
  ...
  const timelineItems = useMemo(() => {
    return orderedComments.map<TimelineItemProps>(comment => ({
      key: comment.id,
      title: `${(dayjs(comment.created_at).format("YYYY-MM-DD HH:mm"))} (${usersByID[comment.created_by]?.username})`,
      content: renderTextWithLineBreaks(comment.comment, true)
    }))
    ...
    <Row justify="center">
      <Col span={orderedComments.length > 0 ? 24 : 1}>
        <div ref={timelineRef} style={{ paddingTop: "1rem" }}>
          {comments.length > 0 ?
            <Timeline mode={"start"} style={{marginLeft: timelineMarginLeft}} items={timelineItems} />
