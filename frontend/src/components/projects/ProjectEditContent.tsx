...
import { Alert, AutoComplete, Button, DatePicker, Form, Input, Space, Switch } from "antd"
...
const ProjectEditContent = () => {
  ...
  const [formData, setFormData] = useState<FormData | undefined>(
    deserialize(isAdding ? EMPTY_PROJECT : project),
  )
   ...
   */
  const listParentProjects = useCallback(
    (input) => {
  ...
  const onFocusExternalId = (ev) => {
    onSearchExternalId(ev.target.value)
  }
      ...
      if (!values.external_id) {
        ...
        listParentProjects(values.external_id).then((response) => {
          ...
          const newData = {
            ...
            external_name: currentExternalName,
          }
          setFormData(deserialize(newData))
        })
      }
  ...
  const onSubmit = () => {
    ...
    if (!isAdding) data.id = Number(id)
    const action = isAdding
      ? dispatch(add(data)).then((project) => {
          history(`/projects/${project.id}`)
        })
      : dispatch(update(Number(id), data)).then(() => {
          history(`/projects/${id}`)
        })
    action
      .then(() => {
  ...
  const title = id === undefined ? "Add Project" : `Update Project ${project ? project.name : id}`
      ...
      <PageContent>
        ...
        >
          ...
          <Form.Item
            label="External ID"
            {...props("external_id")}
            rules={isAdmin ? externalIdRules : requiredRules.concat(externalIdRules)}
...
function deserialize(values: Partial<FMSProject> | undefined): FormData | undefined {
  ...
  if (!newValues.status || newValues.status === "Closed") newValues.status = false
  else newValues.status = true

  if (newValues.targeted_end_date) newValues.targeted_end_date = dayjs(newValues.targeted_end_date)
...
function serialize(form: FormInstance<FormData>): FMSProject {
  ...
  if (form.getFieldValue("name")) newValues.name = form.getFieldValue("name")

  if (!form.getFieldValue("external_id") || form.getFieldValue("external_id").length == 0)
    newValues.external_id = null
  else newValues.external_id = form.getFieldValue("external_id")
  ...
  if (form.getFieldValue("status") === false) newValues.status = "Closed"
  ...
  else newValues.targeted_end_date = null

  if (form.getFieldValue("comment")) newValues.comment = form.getFieldValue("comment")
