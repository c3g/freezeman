...
export function SampleRename() {
  ...
  useEffect(() => {
    dispatch(api.pooledSamples.template.actions()).then((response) => {
      setTemplateAction(response.data.find((action) => action.name === "Rename Sample"))
    })
  }, [dispatch])
  ...
  useEffect(() => {
    dispatch(api.pooledSamples.prefill.templates()).then((response) => {
      setTemplatePrefill(
        response.data.find(
          (prefill) => prefill.description === "Template to rename sample (and its alias)",
        ),
      )
    })
  }, [dispatch])
  ...
  return (
    <PooledSamples
      ...
      beforeTable={
        <Alert
          title="Pools cannot be renamed using this tool. Only individual and pooled samples."
          type="warning"
        />
      }
    />
