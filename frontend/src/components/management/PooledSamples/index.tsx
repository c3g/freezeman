...
export function PooledSamples({
  ...
  const defaultPageSize = useAppSelector((state) =>
    selectCurrentPreference(state, "table.sample.page-limit"),
  )
  const fetchPooledSamples = useCallback<FetchRowData<PooledSampleColumnID, FMSPooledSample>>(
    async ({ pageNumber, pageSize, filters, sortBy }) => {
      const response = await dispatch(
        api.pooledSamples.list(
          {
  ...
  const debouncedOnSort = useCallback(
    (newSortBy: Partial<Record<PooledSampleColumnID, "ascend" | "descend">>) => {
      fetchRowData({ sortBy: newSortBy, pageNumber: 1 }, DEBOUNCE_DELAY)
    },
    [fetchRowData],
  )
  const [tableSortByProps, { sortBy }] = useTableSortByProps<PooledSampleColumnID, FMSPooledSample>(
    debouncedOnSort,
  )

  const [
    smartSelectionProps,
    { resetSelection, defaultSelection, exceptedItems, totalSelectionCount },
  ...
  const debouncedOnFilter = useCallback(
    (newFilters: Filters<PooledSampleColumnID>) => {
      resetSelection()
  ...
  const columnDefinitions = useMemo(() => {
    return Object.entries(COLUMN_DEFINITIONS).reduce(
      (acc, [columnID, definition]) => {
        ...
        totalSelectionCount > 0
          ? {
              ...
              ...smartQuerySetLookup(
                "id",
                defaultSelection,
                exceptedItems.map((id) => Number(id)),
              ),
            }
          : { id__in: "0" } // id/pk 0 does not exist in our database
      return dispatch(api.pooledSamples.prefill.request(options, template))
    },
