...
export const ADD                   = createNetworkActionTypes("DATASET_FILES.ADD");
...
export const add = datasetFile => async (dispatch, getState) => {
    if (getState().datasetFiles.isFetching)
        return;

    return await dispatch(networkAction(
        ADD, api.datasetFiles.add(datasetFile), { meta: { ignoreError: 'APIError' } }));
};
