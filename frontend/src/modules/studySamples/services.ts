...
import serializeFilterParamsWithDescriptions, {
  serializeSortByParams,
} from "../../components/pagedItemsTable/serializeFilterParamsTS"
import {
...
import {
  selectLibrariesByID,
  selectSamplesByID,
  selectStudySettingsByID,
  selectStudyTableStatesByID,
} from "../../selectors"
import store from "../../store"
import api from "../../utils/api"
import {
  fetchLibrariesForSamples,
...
export async function loadStudySamplesInStepByStudy(
  studyID: FMSId,
  stepOrderID: FMSId,
  limit: number,
): Promise<Pick<StudySampleStep, "ready" | "completed" | "removed">> {
...
export function lazyLoadStudySamplesInStepByStudy(studyID: FMSId, stepOrderID: FMSId) {
  return {
    ready: async (offset: number, limit: number) => {
      const fetchSamplesAtStepOrderResponse = await fetchSamplesAtStepOrder(
        studyID,
        stepOrderID,
        offset,
        limit,
      ...
      const readySamples = await fetchSamplesAndLibrariesAndIdentities(
        sampleNextSteps.map((s) => s.sample),
      )

      return {
        samples: readySamples,
        count: readyCount,
        sampleNextStepByID: sampleNextSteps.reduce(
          (sampleNextStepByID, sampleNextStep) => {
    ...
    completed: async (offset: number, limit: number) => {
      const { result: completedSamples, count: completedCount } = await fetchCompletedSamples(
        studyID,
        stepOrderID,
    ...
    removed: async (offset: number, limit: number) => {
      const { result: dequeuedSamples, count: dequeuedCount } = await fetchCompletedSamples(
        studyID,
        stepOrderID,
        ["DEQUEUE_SAMPLE"],
        limit,
...
export async function loadStudySampleStep(
  studyID: FMSId,
  ...
  const workflow = (await fetchWorkflows([study.workflow_id])).find(
    (wf) => wf.id === study.workflow_id,
  )
  if (!workflow) {
...
export async function fetchSamplesAndLibrariesAndIdentities(
  sampleList: number[],
  forceFetch = false,
) {
    ...
    if (samples.length > 0) {
      ...
      const results = (
        await store.dispatch(
          api.sampleIdentity.list({ biosample__in: samples.map((s) => s.biosample_id).join(",") }),
        )
      ).data.results
...
export async function fetchCompletedSamples(
  studyID: FMSId,
  stepOrderID: FMSId,
  ...
  const stepHistoryResponse = await store.dispatch(
    api.stepHistory.getCompletedSamplesForStudy(studyID, {
      step_order__id__in: stepOrderID,
      limit,
      offset,
      workflow_action: qsWorkflowActions,
    }),
  )
  if (!stepHistoryResponse.data) {
    throw new Error(
      `Failed to fetch completed samples for study #${studyID} and step_order #${stepOrderID}`,
    )
  }
  ...
  const completedStudySamples = completedSamples.map((completedSample) => {
    ...
    const user =
      process && process.created_by
        ? usersByID[process.created_by]
        : completedSample.created_by
          ? usersByID[completedSample.created_by]
          : undefined
    ...
    return {
      ...
      executionDate: processMeasurement
        ? processMeasurement?.execution_date
        : timestampStringAsDate(completedSample.created_at),
...
async function fetchSamplesAtStepOrder(
  studyID: FMSId,
  stepOrderID: number,
  offset: number,
  limit: number,
  ...
  return store
    .dispatch(
      api.sampleNextStepByStudy.getStudySamplesForStepOrder(studyID, stepOrderID, {
        ...options,
        limit,
        offset,
      }),
